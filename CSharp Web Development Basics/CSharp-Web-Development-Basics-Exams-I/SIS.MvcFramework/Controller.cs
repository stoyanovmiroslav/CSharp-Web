...
namespace SIS.MvcFramework
{
    ...
    {
        protected const string AUTH_KEY = ".auth-cookie";
        ...
        {
            if (!cookieCollection.ContainsCookie(AUTH_KEY))
            {
                return new MvcUserInfo();
            }

            var cookie = cookieCollection.GetCookie(AUTH_KEY);
        ...
        protected IHttpResponse View(string viewName = null, string layoutName = LAYOUT)
        {
            return this.View(viewName, (object)null, layoutName);
        }
        
        protected IHttpResponse View<T>(T model = null, string layoutName = LAYOUT)
            where T : class
        {
        ...
        protected IHttpResponse BadRequestErrorWithView<T>(string errorMessage, T model, string layoutName = LAYOUT)
        {
            ...
            var layoutFileContent = System.IO.File.ReadAllText($"{VIEWS_FOLDER_PATH}{layoutName}.html");
        ...
        private string GetViewContent<T>(string viewName, T model, string layoutName = LAYOUT)
        {
            var content = this.ViewEngine.GetHtml(viewName,
                System.IO.File.ReadAllText(VIEWS_FOLDER_PATH + viewName + ".html"), model, this.User);
